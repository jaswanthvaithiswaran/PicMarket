        ...
        <Base className="bg-grey-700 mb-5">
            <div className="searchgrid grid grid-cols-3">
                    ...
                    <div className="searchbar mt-2 flex ml-9 ">
                        <input type="search" className="form-control relative flex-auto min-w-0 block w-[200px] md:w-60 md:h-10 px-5 py-1 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" placeholder="Search templates/tags" aria-label="Search" aria-describedby="button-addon2" onChange={handleChange("moviename")}/>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mb-5">
          ...
          templates.map((template,index)=>{
              return(
                ...
                templates.map((template,index)=>{
                    let stringmatch =template._tags.toLowerCase().search(searchInput);
                    if(stringmatch!==-1)
                    {
                        return(
                            <div className="ml-[45px] mr-[45px] mt-[40px]" key={index}>
